        ...
        if (currentAuthTab === 'login') {
            ...
            database.ref('users/' + user).once('value').then((snapshot) => {
                if(snapshot.exists()) {
                    ...
                    const newUserData = { password: pass, name: user, bio: "Member baru Yapping Space ygy", avatar: "https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?w=150", following: {} };
...
// A. Fungsi Klik Profil
window.viewProfile = function(username) {
    if (!username || username === "Anonymous") return;
    
    // Langsung fetch data user
...
// B. Fungsi Pencarian (Panggil ini di event listener input)
window.searchUsers = function(query) {
    const list = document.getElementById('search-results');
    if (!query) { list.innerHTML = ""; return; }
    
    firebase.database().ref('users').orderByKey().startAt(query).endAt(query + "\uf8ff")
    .once('value').then(snapshot => {
        ...
        snapshot.forEach(child => {
            ...
            div.style.cssText = "margin:5px 0; padding:10px; display:flex; gap:10px; align-items:center; cursor:pointer;";
            div.onclick = () => window.viewProfile(username);
            div.innerHTML = `
                <img src="${user.avatar}" width="40" style="border-radius:50%">
                <div><div style="color:#fff">${user.name}</div><div style="color:#8a8894; font-size:0.8rem">@${username}</div></div>
            `;
...
// C. Hubungkan ke Input Pencarian
document.getElementById('search-input').addEventListener('input', (e) => {
    // Jalankan pencarian user
    window.searchUsers(e.target.value);
    // Tetap jalankan filter quotes (fungsi lama)
